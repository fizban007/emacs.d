...
| flycheck-rust           | Flycheck plugin for the Rust language.                   |
...
| moe-theme               | Theme which is very moe :)                               |
| multiple-cursors        | Edit file at multiple cursors at the same time.          |
...
| rust-mode               | Programming mode for the Rust language.                  |
...
#+BEGIN_SRC emacs-lisp
  ;; (add-to-list 'initial-frame-alist '(font . "Source Code Pro-9"))
  ...
  (add-to-list 'initial-frame-alist `(height . ,my-font-size))
  (add-to-list 'default-frame-alist `(height . ,my-font-size))
  (setq line-spacing 2)
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'after-make-frame-functions
            (lambda (frame)
              (my-load-theme frame)))
  (remove-hook 'after-make-frame-functions 'tabbar-install-faces)
#+END_SRC
...
Another very powerful output option is through pandoc, which can
convert the org file to virtually any output form:
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
      ...
      (add-hook 'org-mode-hook (lambda ()
                                 ...
                                 ;; number font
                                 (set-face-attribute 'linum nil :family my-font-family :height my-font-size)))
...
#+BEGIN_SRC emacs-lisp
  (use-package projectile
    ...
    (progn
      ...
      ;; This is a temporary fix for "Symbol's value as variable is void: grep-find-ignored-files" error when invoking ag from projectile:
...
** Multiple Cursors
=multiple-cursors= is a package to provide functionality similar to
that in the SublimeText editor.
#+BEGIN_SRC emacs-lisp
...
Here is configuration for rtags, and it falls back to gtags when rtags
doesn't have the project indexed:
#+BEGIN_SRC emacs-lisp
  ...
  ;; (define-key evil-normal-state-map (kbd "M-.") (function tags-find-symbol-at-point))
  ;; (define-key evil-normal-state-map (kbd "M-,") (function tags-find-references-at-point))
  ;; (define-key evil-normal-state-map (kbd "M-;") (function tags-find-file))
  ;; (define-key evil-normal-state-map (kbd "C-.") (function tags-find-symbol))
  ;; (define-key evil-normal-state-map (kbd "C-,") (function tags-find-references))
  ;; (define-key evil-normal-state-map (kbd "C-<") (function rtags-find-virtuals-at-point))
  ;; (define-key evil-normal-state-map (kbd "M-i") (function tags-imenu))
#+END_SRC
...
** Haskell Mode
The following snippet configures the Haskell programming mode. Most notably it turns on the indentation for Haskell code.
#+BEGIN_SRC emacs-lisp
  (add-hook 'haskell-mode-hook 'turn-on-haskell-indentation)
#+END_SRC

** Rust mode
The following snippet binds F5 to rust compile and F6 to run the current project.
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'rust-mode-hook
            (lambda ()
              (define-key rust-mode-map (kbd "<f5>") 'rust-save-compile)
              (define-key rust-mode-map (kbd "<f6>") 'rust-save-compile-and-run)
              (setq racer-rust-src-path "~/Programs/rust/src/")
              (setq racer-cmd "~/Downloads/racer/target/release/racer")
              (add-to-list 'load-path "~/Downloads/racer/editors/emacs")
